...
#This function exports the provided array as a CSV to the output path
  ...
  foreach ($finalNodeArray as $line) {
    #PREP CURRENT LINE VALUES
    #use 'unr-desc' field for import as body
    ...
    #process 'lat-lon' UNR ext metadata field into two fields (lat and lon), which is what Drupal expects
    if (!empty($line['lat-lon'])) {
      $latLon_array=explode(",",$line['lat-lon']);
      $line['lat']=trim($latLon_array[0]); #needs trim to avoid it being treated as a string if there is a leading space
      $line['lon']=trim($latLon_array[1]); #needs trim to avoid it being treated as a string if there is a leading space
    }
