...
$output_path = __DIR__ . "/OUTPUT/sky-meta.csv";
...
$patterns = ["unr-desc"        =>    "/^.+?(?=\<br>)/",                                      #unr-desc, get first line
             "site-name"       =>    "/(?<=Site Name\:).+?(?=\<)/",                       #site-name, get after "Site name:" to next <br> tag
             "site-address"    =>    "/(?<=Address\:).+?(?=\<)/"                         #Site address, get after "Site address:" to next <br> tag
            ];
...
foreach ($dcNodes as $index=>$node) {
  ...
  if ($id_prefix!='sky'||$isHyphenated==1) {continue;}  #if we're not on a parent 'neo' node, then skip ahead
  ...
  #original data sometimes did not include city and state, so let's add it so we can run it through our geolocator (FIND-LATLON/04_get-lat-lon.php)
  if (!empty($row['site-address'])&&!preg_match('/Las Vegas, NV/',$row['site-address'])) {
    $row['site-address']=$row['site-address'].", Las Vegas, NV";
  }
